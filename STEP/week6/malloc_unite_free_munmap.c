...
typedef struct my_helper_t {
  simple_metadata_t* keep_ptr;
} my_helper_t;

my_helper_t my_helper;
...
void my_add_to_free_list(simple_metadata_t* metadata) {
  ...
  //                     size
  simple_metadata_t* prev = NULL;
  //check if next_metadata is free
  //if next_metadata == mmap_region, don't unite
  while (metadata && next_metadata != my_helper.keep_ptr){ 
    prev = metadata;
    metadata = metadata->next;
...
void my_check_munmap(){
  if (my_helper.keep_ptr == NULL){
    return;
  }
  int flag = 0;
  simple_metadata_t* metadata = simple_heap.free_head;
  simple_metadata_t* prev = NULL;
  simple_metadata_t* keep_metadata = my_helper.keep_ptr;
  while (metadata){
    if (keep_metadata == metadata){
        flag = 1;
        break;
    }
    prev = metadata;
  ...
  if (flag == 1 && keep_metadata->size >= 4096){
    void* ptr = keep_metadata + 1;
    munmap_to_system(ptr, 4096);
    size_t remaining_size = keep_metadata->size - 4096;
    keep_metadata->size = 4096;
    my_remove_from_free_list(keep_metadata, prev);
    ...
    if (remaining_size > sizeof(simple_metadata_t)) {
        ...
        // ... | metadata | object | metadata | free slot | ...
        //     ^          ^        ^
        //     keep_metadata   ptr      new_metadata
        //                 <------><---------------------->
        //                   size       remaining size
        simple_metadata_t* new_metadata = (simple_metadata_t*)((char*)ptr + 4096);
        new_metadata->size = remaining_size - sizeof(simple_metadata_t);
        new_metadata->next = NULL;
        // Add the remaining free slot to the free list.
